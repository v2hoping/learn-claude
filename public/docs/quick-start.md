...
1. **系统提示词**（最稳定，缓存效率最高）：Agent 身份、行为指引、工具描述 <span class="group relative inline-flex items-center cursor-help text-blue-500 hover:text-blue-700 ml-1"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"></path><path d="M12 17h.01"></path></svg><div class="opacity-0 invisible group-hover:opacity-100 group-hover:visible absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-[480px] p-4 bg-white text-gray-800 text-sm rounded-lg shadow-2xl border border-gray-200 z-50 text-left pointer-events-none transition-all duration-200"><strong class="text-blue-600 block mb-2 font-bold">💡 真实系统提示词示例片段：</strong><code class="block bg-gray-50 p-2.5 rounded text-[11px] text-gray-700 font-mono mb-2 leading-relaxed whitespace-pre-wrap border border-gray-200">You are Claude Code, Anthropic's official CLI for software engineering...<br/><br/># Doing Tasks<br/>- Before reporting a task complete, verify it actually works: run the test, execute the script, check the output.<br/>- If you notice the user's request is based on a misconception, say so. You're a collaborator, not just an executor.<br/><br/># Output Style<br/>- IMPORTANT: Go straight to the point... Be extra concise... Lead with the answer or action, not the reasoning.<br/>- For clear communication with the user the assistant MUST avoid using emojis.</code><div class="text-[11px] text-gray-500 mb-3 pl-1 border-l-2 border-blue-200 leading-relaxed"><strong>[中文翻译]</strong> 你是 Claude Code，Anthropic 官方的软件工程 CLI 工具...<br/><strong># 执行任务</strong><br/>- 在报告任务完成之前，必须验证它是否真实生效：运行测试、执行脚本、检查输出。<br/>- 如果你发现用户的请求是基于某种误解，请直接指出来。你是一个合作者，而不仅仅是一个执行机器。<br/><strong># 输出风格</strong><br/>- 重要：直奔主题... 尽可能简明扼要... 把答案或行动放在最前面，而不是推理过程。<br/>- 为了与用户保持清晰的沟通，助手必须避免使用 Emoji 表情符号。</div><strong class="text-green-600 block mb-1 font-bold">⚡ 缓存机制：</strong>系统提示词（长达数千甚至上万 Token）会被完整发送到 Anthropic API，并命中<strong>服务端全球 KV Cache</strong>。由于这部分内容在整个会话中极少变动，只要不发生变化，你在后续的每一轮对话中都不需要为这些巨量的背景指令重复支付计算时间和 Token 费用。</div></span>
